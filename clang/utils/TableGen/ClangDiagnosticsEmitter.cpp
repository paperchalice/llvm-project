...
static std::string DoXmlEscape(const std::string &S) {
  auto NoAmp = std::regex_replace(S, std::regex("&"), "&amp;");
  auto NoLt = std::regex_replace(std::move(NoAmp), std::regex("<"), "&lt;");
  auto NoGt = std::regex_replace(std::move(NoLt), std::regex(">"), "&gt;");
  return NoGt;
}
void clang::EmitClangDiagsXliffTemplate(const llvm::RecordKeeper &Records,
                                        llvm::raw_ostream &OS,
                                        const std::string &Component) {
  ArrayRef<const Record *> Diags =
      Records.getAllDerivedDefinitions("Diagnostic");
  ArrayRef<const Record *> TextSubstitutions =
      Records.getAllDerivedDefinitions("TextSubstitution");
  OS << llvm::formatv(
      R"(<?xml version="1.0" encoding="UTF-8"?>
<!-- This file is automatically generated. Do not edit this file directly by hand! -->
<xliff version="2.1" xmlns="urn:oasis:names:tc:xliff:document:2.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://docs.oasis-open.org/xliff/xliff-core/v2.1/os/schemas/xliff_core_2.0.xsd">
  <file id="Diagnostic{0}" original="clang/Basic/Diagnostic{0}Kinds.td">
    <group id="TextSubstitution">
)",
      Component);

  const char Unit[] = R"(      <unit id="{0}">
        <segment>
          <source>{1}</source>
          <target></target>
        </segment>
      </unit>)";
  for (const Record &R : make_pointee_range(TextSubstitutions)) {
    StringRef TextSubstitutionComponent = R.getValueAsString("Component");
    if (!Component.empty() && Component != TextSubstitutionComponent)
      continue;
    OS << llvm::formatv(Unit, R.getName(),
                        DoXmlEscape(R.getValue("Substitution")
                                        ->getValue()
                                        ->getAsUnquotedString()))
       << '\n';
  }
  ...
  OS << "    <group id=\"Diagnostic\">\n";
  auto DiagsSize = Diags.size();
  for (const Record &R : make_pointee_range(Diags)) {
    StringRef DiagComponent = R.getValueAsString("Component");
    if (!Component.empty() && Component != DiagComponent)
      continue;
    OS << llvm::formatv(
              Unit, R.getName(),
              DoXmlEscape(
                  R.getValue("Summary")->getValue()->getAsUnquotedString()))
       << '\n';
  }
  OS << R"(
    </group>
